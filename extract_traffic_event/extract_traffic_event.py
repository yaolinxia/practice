...
# 1.3 返回根节点，子节点，层次结构{'超速': ['超速处罚', '超速扣分', '超速吊销驾驶证', '超速罚款']}
...
# 3. 将语料库中的内容，与相应的事件进行匹配，如果符合就把事件提取出来,返回的结果是词典
...
# event_dict:{'超速': ['超速处罚', '超速扣分', '超速吊销驾驶证', '超速罚款']}
def extract_event_format2(event_dict, corpus):
    ...
    for f_event in event_dict:
        ...
        # 先考虑子事件，讲子事件存储为词典形式
        # 4.1 匹配分词后的子事件,根事件也包含在其中
        # print(event_dict[f_event])
        # event_dict[f_event])：是一个列表，需要对列表中的每一个元素进行分词
...
if __name__ == '__main__':
    ...
    json_path = "event_count_1210.json"
    ...
    to_json2(extract_event_format2(event_corpus_dict, corpus_list), json_path)
